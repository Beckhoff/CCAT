...
{
	memcpy_fromio(skb->data, fifo->next_iomem->data, len);
}
...
{
	const struct frame_dma *frame = priv->tx_fifo.next_dma;
...
{
	struct frame_dma *const frame = fifo->next_dma;
